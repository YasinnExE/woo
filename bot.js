...
client.on("messageUpdate", async (old, nev) => {
 ...
 const embed = new Discord.MessageEmbed() .setColor("#ff7e00") .setDescription(`<:carpi:837676050800574505> ${nev.author} , **Mesajını Editleyerek Reklam Yapmaya Çalıştı!**`)
            .addField("Reklamı:",nev)
        
            nev.delete();
            const embeds = new Discord.MessageEmbed() .setColor("#ff7e00") .setDescription(`<:carpi:837676050800574505> ${nev.author} , **Mesajı Editleyerek Reklam Yapamana İzin Veremem!**`) 
...
client.on("message", async msg => {
    ...
    let i = await db.fetch(`reklam.${msg.member.guild.id}.durum`);
     ...
     msg.delete({timeout:750});
                    const embeds = new Discord.MessageEmbed() .setColor("#ff7e00") .setDescription(`<:carpi:837676050800574505> <@${msg.author.id}> , **Bu Sunucuda Reklam Yapmak Yasak!**`)
      msg.channel.send(embeds).then(msg => msg.delete({timeout: 5000}));
                const embed = new Discord.MessageEmbed() .setColor("#ff7e00") .setDescription(`<:carpi:837676050800574505> ${msg.author} , **Reklam Yapmaya Çalıştı!**`) .addField("Mesajı:",msg)
               client.channels.cache.get(y).send(embed)
                  }              
                } catch(err) {
                  console.log(err);
...
client.on("messageUpdate", async (old, nev) => {
   ...
   if (i) {
      ...
      if (nev.member.hasPermission("BAN_MEMBERS")) return ;
       //if (ayarlar.gelistiriciler.includes(nev.author.id)) return ;
 const embed = new Discord.MessageEmbed() .setColor("#ff7e00") .setDescription(`<:carpi:837676050800574505> ${nev.author} , **Mesajını Editleyerek Küfür Etmeye Çalıştı!**`)
            .addField("Küfür:",nev)
        
            nev.delete();
            const embeds = new Discord.MessageEmbed() .setColor("#ff7e00") .setDescription(`<:carpi:837676050800574505> ${nev.author} , **Mesajı Editleyerek Küfür Etmene İzin Veremem!**`) 
          client.channels.cache.get(y).send(embed)
            nev.channel.send(embeds).then(msg => msg.delete({timeout:5000}));
          ...
          if (i) {
              if (küfür.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                 ...
                 //  if (!ayarlar.gelistiriciler.includes(msg.author.id)) return ;
     msg.delete({timeout:750});
                    const embeds = new Discord.MessageEmbed() .setColor("#ff7e00") .setDescription(`<:carpi:837676050800574505> <@${msg.author.id}> , **Bu Sunucuda Küfür Yasak!**`)
      msg.channel.send(embeds).then(msg => msg.delete({timeout: 5000}));
                const embed = new Discord.MessageEmbed() .setColor("#ff7e00") .setDescription(`<:carpi:837676050800574505> ${msg.author} , **Küfür Etmeye Çalıştı!**`) .addField("Mesajı:",msg)
               client.channels.cache.get(y).send(embed)
                  }              
                } catch(err) {
                  console.log(err);
...
client.on("guildMemberAdd", async member => {
  ...
  if (member.guild.memberCount == sayaç) {
    return client.channels
      .get(kanal)
      .send(
        `<:emoji11:837449766265684058> **Sayaç Sıfırlandı!** \`${member.guild.memberCount}\` **Kişiyiz!**`
      );
...
member.roles.add(rol)
  client.channels.cache.get(kanal).send('<:9693_Idle_Status:837675996798124082> Otomatik Rol Verildi Seninle Beraber **`'+member.guild.memberCount+'`** Kişiyiz! Hoşgeldin! **`'+member.user.username+'`**')
...
const kanal = "837082915107504223" //Eklendim mesajının atılacağı kanal ID'sini giriniz.
...
client.on("message", msg => {
	...
	.setColor("RANDOM")
	.setDescription(`**Dostum Prefiximi Ögrenmekmi İstiyorsun.**\nO Zaman Söylim Prefixim = !\n Bilgi Almak İcin İse = !yardım`)
...
client.channels.cache.get(kanal).send(`<:emoji11:837449766265684058> ${member} Adlı Kullanıcı Aramıza Katıldı. Kullanıcıyı Davet Eden  ${daveteden}  Toplam **${davetsayi}** Daveti Oldu`)  
...
client.on("guildMemberRemove", async member => {
  ...
  if (!davetçi) {
    return client.channels.cache.get(kanal).send(`<:carpi:837676050800574505> ${member} Adlı Kullanıcı Aramızdan Ayarıldı Davet Eden Bulunamadı!`);
  } else {
     
client.channels.cache.get(kanal).send(`<:carpi:837676050800574505> ${member} Adlı Kullanıcı Aramızadan Ayrıldı Kullanıcıyı Davet Eden ${daveteden}  Toplam  **${davetsayi}** Daveti Kaldı`)  
...
client.on("guildMemberAdd", member => {
  ...
  if (ayyy < 1) {
    kontrol = "**Şüpheli** <:carpi:837676050800574505>";
  }
  if (ayyy > 1) {
    kontrol = "**Güvenilir** <:emoji13:837449794247065653>";
  }
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .setDescription(
      `<a:elmas:837449699015262238> **Hoşgeldin!** ${
        member.user
      }, seninle beraber **${
        guild.memberCount
      }** kişi olduk! \n <:kodpencresi:837676063118983199> Kaydının yapılması için  **İsim** ve **Yaş** Yazman Gerek. \n <:6007_Toucan:837676053719416862> Hesap Kuruluş: **${moment(
        user.createdAt
      ).format("DD")} ${aylar[moment(user.createdAt).format("MM")]} ${moment(
        user.createdAt
      ).format(
        "YYYY HH:mm:ss"
       )}** \n<a:elmas:837449699015262238> Bu vatandaş: ${kontrol} \n <a:elmas:837449699015262238> <@&${kayıtçı}> Rolundeki Yetkililer Sizinle İlgilecektir
...
.setDescription('<a:elmas:837449699015262238> Aleyküm Selam. Hoş Geldin!')
.setTimestamp()
.setFooter('OneWoo')
.setColor(0x36393E)
...
const voiceChannel = "837445725637312563"
...
Botumuzdaki özelliklere daha hızlı ulaşabilmek için https://discord.gg/EtZuYFE3JZ sunucumuza gelebilirsin.

( __Bu Mesajı Tek Sunucu Sahibine Yolladım.__ )
`)
})
//müzik sistemi
...
const emmmmbed = new Discord.MessageEmbed()
  ...
  .addField(
    `OneWoo | Teşekkürler`,
    `**Selamlar, Ben GameMoonTR™ (OneWoo Bot'un Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`
  )
  .addField(
    `OneWoo | Prefix`,
...
client.on("message", async message => {
    ...
    if (cmd === "yttogether") {
        ...
        fetch(`https://discord.com/api/v8/channels/${channel.id}/invites`, {
            ...
            .then(invite => {
                ...
                message.channel.send(`Başlatmak İçin Tıklayın! ${channel.name}: <https://discord.gg/${invite.code}>`);
            })
            .catch(e => {
                message.channel.send("başlatılamadı!");
            })
    }
    ...
    if (cmd === "yayın") {
        ...
        if (!channel || channel.type !== "voice") return message.channel.send("**!yayın \`[ Kanal İd ]\` youtube \n !yayın \`[ Kanal İd ]\` betrayal**");
        if (!channel.permissionsFor(message.guild.me).has("CREATE_INSTANT_INVITE")) return message.channel.send("CREATE_INSTANT_INVITE iznine ihtiyacım var");
